<###################################################################################
  ...
  Notepad++ Installation Validation
...
if (Test-Path "HKLM:SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\notepad++.exe") {
    ...
    $fileVersion = (Get-Item (Get-ItemProperty 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\notepad++.exe').'(Default)').VersionInfo.FileVersion
